...
// Wallet is a minimal single-key BCH wallet used to fund and sign registry
...
// LoadOrCreateWallet loads the wallet key from ~/.freedom/bch.key, creating
// one on first use (mode 0600, same pattern as the node identity key).
func LoadOrCreateWallet(network string, client *ElectrumClient) (*Wallet, error) {
	path, err := bchKeyPath()
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) script() []byte { return p2pkhScript(w.pkh) }
...
func (w *Wallet) utxos(ctx context.Context) ([]walletUTXO, error) {
...
// Network returns the chain this wallet is bound to ("mainnet", "chipnet", …).
func (w *Wallet) Network() string { return w.network }

// Holdings reports spendable balance and how many name NFTs the wallet holds,
// from a single UTXO scan. Token classification stays in this package so
// callers never have to reason about CashTokens capabilities.
func (w *Wallet) Holdings(ctx context.Context) (balanceSat int64, nameNFTs int, err error) {
	...
	for _, u := range utxos {
		switch {
		case u.token != nil && u.token.Capability != tokenCapabilityNone:
			nameNFTs++
		case u.token == nil:
			balanceSat += u.value
		}
	}
	return balanceSat, nameNFTs, nil
}
...
func (w *Wallet) BuildClaimTx(ctx context.Context, label string, ownerPub []byte) ([]byte, error) {
	utxos, err := w.utxos(ctx)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) BuildRebindTx(ctx context.Context, label string, category, newOwnerPub []byte) ([]byte, error) {
	utxos, err := w.utxos(ctx)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) newTx(inputs []walletUTXO) (*transaction, []*secp256k1.PrivateKey) {
