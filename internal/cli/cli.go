...
func cliSet(args []string) error {
	...
	newRR := record.RR{Type: rtype, Value: value, TTL: ttl}
	updated := false
	for i := range records {
	...
	tmp := &record.FNRecord{Label: label, Records: records}
...
func fetchCurrentRecord(api, fullName string) (*record.FNRecord, bool) {
...
func loadStaged(label string) ([]record.RR, error) {
...
func saveStaged(label string, records []record.RR) error {
	path, err := stagePath(label)
	if err != nil {
		return err
	}
