...
func TestContentWireFraming(t *testing.T) {
	...
	if _, err := readBlob(&empty); err != content.ErrBlobNotFound {
		t.Fatalf("expected content.ErrBlobNotFound for zero blob, got %v", err)
	}
}
...
func TestContentStreamTransfer(t *testing.T) {
	...
	if h, _ := content.ContentHash(got); h != hash {
