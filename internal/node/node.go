...
import (
	...
	p2precord "github.com/libp2p/go-libp2p-record"
...
// The interface this node satisfies for the HTTP API lives with its consumer,
// in internal/httpapi (httpapi.FreedomDHT). Go satisfies interfaces
// structurally, so *FreedomNameNode implements it without either package
// importing the other — and httpapi stays free of the libp2p dependency tree.
...
type FreedomNameNode struct {
	...
	owned   map[string]*record.FNRecord
	ownedMu sync.Mutex
...
func NewNode(ctx context.Context, cfg *config.Config) *FreedomNameNode {
	...
	dhtOpts := []dht.Option{
		...
		dht.Resiliency(2),
		dht.Validator(p2precord.NamespacedValidator{
			"fn": record.FreedomNameValidator{},
		}),
	}
	...
	freedomName := &FreedomNameNode{
		...
		owned:            make(map[string]*record.FNRecord),
	}
...
func (freedomName *FreedomNameNode) AttachContent(store *content.BlobStore, cfg *config.Config) *ContentService {
...
// ------------------------------------------------------------
// TODO: Move all the methods below to `stats.go` or something? The interface
// they satisfy now lives with its consumer as httpapi.FreedomDHT, and could be
// narrowed to a Stats interface once these are grouped.
// ------------------------------------------------------------
