...
func TestPushOfferFraming(t *testing.T) {
	root, _ := content.ContentHash([]byte("some content"))
	...
	var b4 bytes.Buffer
	writePushOffer(&b4, root, content.MaxContentSize+1, 1) // over max
...
func newPushTestPeer(t *testing.T, budget int64) *pushTestPeer {
	...
	cs := &ContentService{
		...
		index:       ix,
		hostBudget:  budget,
		maxPushSize: content.MaxContentSize,
...
func TestPushChunkedSet(t *testing.T) {
	...
	data := testsupport.TestBytes(content.ChunkSize + 999) // manifest + 2 chunks
...
func TestPushDeclinedOverBudget(t *testing.T) {
	...
	root, err := sender.cs.Put(context.Background(), testsupport.TestBytes(4096))
	if err != nil {
		t.Fatalf("put: %v", err)
	}
	status, err := pushBetween(t, sender, receiver, root)
...
func TestPushCorruptBlobRejected(t *testing.T) {
	...
	// A blob's filename is its hash, so this is the file the store would read.
	if err := os.WriteFile(filepath.Join(sender.cs.store.Dir(), root), []byte("evil bytes"), 0600); err != nil {
